...
class CivitasJuego
  ...
  def initialize(nombres=[])
    ...
    for i in (0..nombres.size-1)
      ...
      nuevo_jugador = Jugador.new(nombres[i])
  ...
  def avanza_jugador
        ...
        posicion_actual = @jugadores[@indice_jugador_actual].get_casilla_actual
        ...
        casilla.recibeJugador(@indice_jugador_actual, @jugadores)
  ...
  def cancelar_hipoteca(ip)
    return @jugadores.get(@indice_jugador_actual).cancelarHipoteca(ip);
  end
  ...
  def construir_casa(ip)
    return @jugadores.get(@indice_jugador_actual).construir_casa(ip)
  end
  
  def construir_hotel(ip)
    return @jugadores.get(@indice_jugador_actual).construir
  end
  ...
  def info_jugador_texto
    ...
    info = "Nombre del jugador: #{nuevo.nombre} Esta en la casilla: #{get_casilla_actual} Tiene #{nuevo.propiedades.size}  
  ...
  def siguiente_paso
        ...
        jugador_actual= Jugador.new(@jugadores.at(@indice_jugador_actual))
